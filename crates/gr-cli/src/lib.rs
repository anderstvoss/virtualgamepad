...
// Phase 11 automated portion. The spec also lists a real-device comparison
// (hardware-gated) and `vgpd-demo phase-gate 11` itself; neither is a runnable
// gate command here, so only the deterministic checks are wired.
...
mod tests {
    ...
    #[test]
    fn phase_eleven_commands_match_expected_order() {
        let commands = phase_gate_commands(11).expect("phase 11 commands");
        let expected = PHASE_11_COMMANDS
            .iter()
            .map(|command| {
                command
                    .iter()
    ...
    #[test]
    fn phase_eleven_commands_match_plan_spec() {
        let repo_root = repo_root().expect("workspace root");
        let plan_path = repo_root.join("docs/spec/implementation/RUST_IMPLEMENTATION_PLAN.md");
        let plan = std::fs::read_to_string(plan_path).expect("read implementation plan");
        let phase_eleven = plan
            .split("## Phase 11:")
            .nth(1)
            .and_then(|section| section.split("## Phase 12:").next())
            .expect("phase 11 section");
        let automated = phase_eleven
            .split("Automated portion:")
            .nth(1)
            .and_then(|section| section.split("Manual portion:").next())
            .expect("automated section");

        for command in PHASE_11_COMMANDS
            .iter()
            .map(|command| format!("`{}`", command.join(" ")))
        {
            assert!(
