...
const BACKEND_ID: &str = "macos-hid";
const PLANNING_ONLY_NOTE: &str =
    "phase-12 macos-hid backend is planning-only; no device realization yet";
const DEPLOYMENT_REQUIREMENT: &str = "deployment requirement: a notarized DriverKit system extension and the matching app entitlement must be approved before realization";

#[derive(Debug, Clone, Default)]
pub struct MacosHidBackendFactory;

impl MacosHidBackendFactory {
    #[must_use]
    pub fn new() -> Self {
        Self
    }
...
impl BackendFactory for MacosHidBackendFactory {
    ...
    fn can_realize(&self, request: &BackendRealizationRequest) -> BackendSupportReport {
        let host_supported = request.host_platform == HostPlatform::Macos;
        ...
        if !host_supported {
            notes.push("requested host platform does not match macos-hid".to_string());
        }
        if !profile_supported {
            notes.push(format!(
                "profile `{}` is not registered, so the Phase 12 macOS HID foundation cannot plan it",
                request.profile_id
            ));
        }
        if request.requested_fidelity_tier == FidelityTier::HardwareFaithful {
            notes.push(
                "hardware-faithful transport behavior is out of scope for the Phase 12 macos-hid provider foundation"
                    .to_string(),
            );
        } else if request.requested_fidelity_tier == FidelityTier::Compatibility {
            notes.push(
                "compatibility-tier downgrade targets are not modeled by the Phase 12 macos-hid provider foundation"
                    .to_string(),
            );
        }
    ...
    fn open_session(
        ...
        Err(BackendError::Unsupported {
            reason:
                "macos-hid realization is not implemented in Phase 12; this provider is planning-only"
                    .to_string(),
        })
    }
}

pub struct MacosHidBackendSession {
    session_id: SessionId,
    state: BackendState,
    last_error: Option<String>,
}

impl MacosHidBackendSession {
    #[allow(dead_code)]
...
impl BackendSession for MacosHidBackendSession {
...
mod tests {
    ...
    fn inventory_entry_matches_phase_twelve_contract() {
        let entry = MacosHidBackendFactory::new().inventory_entry();
        assert_eq!(entry.backend_id.as_ref(), BACKEND_ID);
        assert_eq!(entry.family, BackendFamily::MacosHid);
        assert_eq!(entry.level, BackendLevel::Hid);
        assert_eq!(entry.host_platform, HostPlatform::Macos);
        assert_eq!(
    ...
    #[test]
    fn identity_aware_macos_request_is_supported() {
        let support = MacosHidBackendFactory::new()
            .can_realize(&request(FidelityTier::IdentityAware, HostPlatform::Macos));
    ...
    #[test]
    fn hardware_faithful_macos_request_is_limited() {
        let support = MacosHidBackendFactory::new().can_realize(&request(
    ...
    fn open_session_is_explicitly_unsupported() {
        let result = MacosHidBackendFactory::new().open_session(&BackendOpenContext {
