...
| --- | --- | --- |
| A: service contract | API hazard. All four curated handles expose `poll_output`, readiness, write interest and relative deadlines. `common/session.rs` services required HID work through output draining. The name suggests an optional observer operation. | Add a clearly documented service entry point while retaining source compatibility. Specify idle service, bounded callbacks, readiness plus timers, and immediate deadlines. Keep executor ownership with callers (AR-28). |
| B: edit contention | API hazard. `demo/src/gui.rs` shares `Arc<Mutex<Controller>>`; drawing holds mutable controller access. Nonblocking GUI acquisition does not bound how long an acquired lock delays service. | Prove single service-owner integration using bounded native edit commands and detached UI snapshots. Do not move demo workers into the library or collapse press/release transitions into an unchecked latest-state mailbox. |
| C/D: persistent identity | Missing API capability. `common/session.rs` generates per-creation identity before provider open; Sony personalities retain it, but callers cannot restore a typed emulated identity. | Design controller-owned identity types and explicit opt-in restoration. Keep current ephemeral behavior until the default is decided. Fresh transport/request state must accompany every recreation; storage stays caller-owned (AR-8–11, AR-21). |
...
6. **Future complexity exercise:** use a hypothetical multi-component controller
